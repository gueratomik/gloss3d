...
static uint32_t getSeparator_r ( GTK3G3DUIOBJECTLIST *gtk3objlist,
...
static PICKEDOBJECT *selectObject_r ( GTK3G3DUIOBJECTLIST *gtk3objlist,
                                      GtkStyleContext      *context,
                                      G3DOBJECT            *obj,
...
/******************************************************************************/
static GTK3G3DUIOBJECTLIST *gtk3_g3duiobjectlist_new ( GTK3G3DUI *gtk3gui ) {
    GTK3G3DUIOBJECTLIST *gtk3ols = calloc ( 0x01, sizeof ( GTK3G3DUIOBJECTLIST ) );

    if ( gtk3ols == NULL ) {
        fprintf ( stderr, "%s: calloc failed\n", __func__ );
    }

    g3duiobjectlist_init ( &gtk3ols->core, &gtk3gui->core );
...
static void Input ( GtkWidget *widget,
    ...
    GTK3G3DUIOBJECTLIST   *gtk3ols = ( GTK3G3DUIOBJECTLIST * ) user_data;
    G3DUI           *gui = gtk3ols->core.gui;
    ...
    switch ( gdkev->type ) {
        ...
        case GDK_2BUTTON_PRESS : {
            ...
            uint32_t xsep = getSeparator_r ( gtk3ols,
                                             context,
                             ( G3DOBJECT * ) sce,
                                             0x00,
                                             0x00,
...
/*******************************************************************************/
static uint32_t getObjectNameWidth ( GtkStyleContext *context,
                                     G3DOBJECT       *obj ) {
...
/*******************************************************************************/
static uint32_t getSeparator_r ( GTK3G3DUIOBJECTLIST *gtk3objlis,
                                 GtkStyleContext     *context,
                                 G3DOBJECT           *obj,
                                 uint32_t             x,
                                 uint32_t             y,
    ...
    seppos = g3duiobjectlist_initListedObject ( &gtk3objlis->core,
                                                &lob,
                                                 x,
                                                 y,
                                                 strwidth );
...
/******************************************************************************/
static PICKEDOBJECT *selectObject_r ( GTK3G3DUIOBJECTLIST *gtk3objlis,
                                      GtkStyleContext     *context,
                                      G3DOBJECT           *obj,
                 /* Object X pos  */  uint32_t             x,
                 /* Object Y pos  */  uint32_t             y,
    ...
    /*if ( obj->type != G3DSCENETYPE ) {*/
        pob = g3duiobjectlist_pickObject ( &gtk3objlis->core,
                                            x,
                                            y,
                                            xsep,
                                            xm,
    ...
    if ( ( obj->flags & OBJECTCOLLAPSED ) == 0x00 ) {
        ...
        while ( ltmpobj ) {
            ...
            if ( ( sel = selectObject_r ( gtk3objlis,
                                          context,
                                          child,
                                          x + xindent,
                                          curheight,
...
/******************************************************************************/
static void drawActive ( GtkStyleContext *context,
                         cairo_t         *cr,
                         G3DOBJECT       *obj, 
                         uint32_t         x, 
                         uint32_t         y ) {
...
/******************************************************************************/
static void drawVisible ( GtkStyleContext *context,
                          cairo_t         *cr,
                          G3DOBJECT       *obj, 
                          uint32_t         x, 
                          uint32_t         y ) {
...
/******************************************************************************/
static void drawTextures ( GtkStyleContext *context,
                           cairo_t         *cr,
                           G3DOBJECT       *obj, 
                           G3DUIRECTANGLE  *rec ) {
    ...
    if ( obj->type & MESH && ( ( obj->type & MODIFIER ) == 0x00 ) ) {
        ...
        while ( ltmptex ) {
            ...
            g3duimaterialmap_buildSphere ( matmap, tex->mat, 0.8f );

            g3duimaterialmap_fillData ( matmap, tex->mat, pixel );
...
/******************************************************************************/
static void drawUVMaps ( GtkStyleContext *context,
                         cairo_t         *cr,
                         G3DOBJECT       *obj, 
                         G3DUIRECTANGLE  *rec ) {
...
/******************************************************************************/
static void drawObjectIcon ( GtkStyleContext *context,
                             cairo_t         *cr,
                             G3DOBJECT       *obj, 
                             uint32_t         x, 
                             uint32_t         y ) {
    ...
    if ( pemico == NULL ) pemico = gdk_pixbuf_new_from_xpm_data ( pemico_xpm );
   /***************************************************************************/
    ...
    switch ( obj->type ) {
        ...
        case G3DPARTICLEEMITTERTYPE : objico = pemico; break;
...
/******************************************************************************/
static void drawArrow ( GtkStyleContext *context,
                        cairo_t         *cr,
                        G3DOBJECT       *obj, 
                        uint32_t         x, 
                        uint32_t         y ) {
...
/******************************************************************************/
static uint32_t printObject_r ( GTK3G3DUIOBJECTLIST *gtk3objlist,
                                GtkStyleContext     *context,
                                cairo_t             *cr, 
                                G3DOBJECT           *obj, 
                                uint32_t             x, 
    ...
    lob = g3duiobjectlist_sizeListedObject ( &gtk3objlist->core,
                                              obj,
                                              x,
                                              y,
                                              xsep,
...
static gboolean Draw ( GtkWidget *widget, 
    ...
    if ( ( gtk3ols->core.oldWidth  != width  ) || 
         ( gtk3ols->core.oldHeight != height ) ) {
        gtk_widget_set_size_request ( widget, width, height );

        gtk3ols->core.oldWidth  = width;
        gtk3ols->core.oldHeight = height;
    }
...
GTK3G3DUIOBJECTLIST *gtk3_g3duiobjectlist_create ( GtkWidget *parent,
    ...
    g_signal_connect ( G_OBJECT (drw), "realize", G_CALLBACK (Realize), gtk3ols );
    g_signal_connect ( G_OBJECT (drw), "destroy", G_CALLBACK (Destroy), gtk3ols );
    g_signal_connect ( G_OBJECT (drw), "draw"   , G_CALLBACK (Draw)   , gtk3ols );

    g_signal_connect ( G_OBJECT (drw), "motion_notify_event" , G_CALLBACK (Input), gtk3ols );
    g_signal_connect ( G_OBJECT (drw), "button-press-event"  , G_CALLBACK (Input), gtk3ols );
    g_signal_connect ( G_OBJECT (drw), "button-release-event", G_CALLBACK (Input), gtk3ols );
    g_signal_connect ( G_OBJECT (drw), "key_press_event"     , G_CALLBACK (Input), gtk3ols );
    g_signal_connect ( G_OBJECT (drw), "key_release_event"   , G_CALLBACK (Input), gtk3ols );
