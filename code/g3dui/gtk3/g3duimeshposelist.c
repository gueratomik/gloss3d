...
static uint32_t drawMeshPoses ( GtkStyleContext *context, 
                                cairo_t         *cr,
                                G3DMESH         *mes ) {
    G3DMESHPOSEEXTENSION *ext = ( G3DMESHPOSEEXTENSION * ) g3dobject_getExtensionByID ( mes, MESHPOSEEXTENSION, 0x00 );
    ...
    if ( ext ) {
        ...
        while ( ltmpmps ) {
            ...
            cairo_text_extents ( cr, mps->name, &te );

            if ( maxwidth < te.width ) maxwidth = te.width;

            gtk_style_context_get_color ( context, GTK_STATE_FLAG_NORMAL, &fg );

            if ( mps->flags & MESHPOSESELECTED ) {
                cairo_set_source_rgba ( cr, 1.0f  , 0.0f    , 0.0f   , fg.alpha );
            } else {
                cairo_set_source_rgba ( cr, fg.red, fg.green, fg.blue, fg.alpha );
            }

            mpsy = y - ( te.height / 2 + te.y_bearing ) + LISTINDENT * 0.5f;

            cairo_move_to   ( cr, 0, mpsy );
            cairo_show_text ( cr, mps->name );
    ...
    switch ( gdkev->type ) {
        ...
        case GDK_BUTTON_PRESS : {
            ...
            if ( obj && ( obj->type == G3DMESHTYPE ) ) {
                G3DMESHPOSE *mps = common_g3duimeshposelist_getMeshPose ( gui, bev->x,
                                                                               bev->y );

                if ( mps ) {
                    common_g3duimeshposelist_selectPoseCbk ( gui, mps );
                }
            }
...
static void Draw ( GtkWidget *widget, cairo_t *cr, gpointer user_data ) {
    ...
    if ( obj && obj->type == G3DMESHTYPE ) {
        ...
        size = drawMeshPoses ( context, cr, mes );
    }
...
*/    
    ...
    /*g_signal_connect ( G_OBJECT (drw), "motion_notify_event" , G_CALLBACK (meshposelistarea_input), gui );*/
