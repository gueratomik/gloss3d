...
#define EDITSYMMETRYGENERAL "Symmetry"
#define EDITSYMMETRYPLANE   "Orientation"
#define EDITSYMMETRYLIMIT   "Merge limit"

/******************************************************************************/
static GTK3G3DUISYMMETRYEDIT *gtk3_g3duisymmetryedit_new ( GTK3G3DUI *gtk3gui ) {
    GTK3G3DUISYMMETRYEDIT *gtk3sed = calloc ( 0x01, sizeof ( GTK3G3DUISYMMETRYEDIT ) );
...
/******************************************************************************/
static void updateGeneralPanel ( GTK3G3DUISYMMETRYEDIT *gtk3sed ) {
    gtk3sed->core.gui->lock = 0x01;

    if ( gtk3sed->core.editedSymmetry ) {
        G3DSYMMETRY *sym = gtk3sed->core.editedSymmetry;

        gtk_spin_button_set_value ( gtk3sed->limitEntry, sym->mergelimit );

        gtk_combo_box_set_active  ( GTK_COMBO_BOX(gtk3sed->orientationCombo), sym->orientation );
    }
...
/******************************************************************************/
static void limitCbk ( GtkWidget *widget,
                       gpointer   user_data ) {
    GTK3G3DUISYMMETRYEDIT *gtk3sed = ( GTK3G3DUISYMMETRYEDIT * ) user_data;
    ...
    ret = g3duisymmetryedit_limit ( &gtk3sed->core, lim );
...
static void planeCbk ( GtkWidget *widget, gpointer user_data ) {
    ...
    ret = g3duisymmetryedit_plane ( &gtk3sed->core, str );
...
void gtk3_g3duisymmetryedit_update ( GTK3G3DUISYMMETRYEDIT *gtk3sed ) {
    ...
    if ( gui->sce ) {
        ...
        if ( nbsel ) {
            ...
            if ( g3dobjectlist_checkType ( sce->lsel, G3DSYMMETRYTYPE ) ) {
                gtk3sed->core.multi = ( nbsel > 0x01 ) ? 0x01 : 0x00;

                gtk3sed->core.editedSymmetry = ( G3DSYMMETRY * ) g3dscene_getLastSelected ( sce );
...
/******************************************************************************/
static void createGeneralPanel ( GTK3G3DUISYMMETRYEDIT *gtk3sed,
                                 gint               x,
                                 gint               y,
                                 gint               width,
                                 gint               height ) {
    ...
    gtk3sed->limitEntry       = ui_createFloatText     ( pan, 
                                                         gtk3sed,
                                                         EDITSYMMETRYLIMIT,
                                                         CLASS_MAIN,
                                                         0.0f, FLT_MAX,
    ...
    gtk3sed->orientationCombo = ui_createPlaneSelector ( pan, 
                                                         gtk3sed, 
                                                         EDITSYMMETRYPLANE,  
                                                         CLASS_MAIN,
                                                         0, 24, 96, 96, 20,
...
static void Realize ( GtkWidget *widget,
    ...
    gtk3_g3duisymmetryedit_update ( gtk3sed );
}
