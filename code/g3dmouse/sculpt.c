...
void sculpt_pick ( G3DSCULPTTOOL *st, G3DMESH *mes,
    ...
    g3dface_evalSubdivision ( fac, mes->subdiv, &nbFaces, &nbEdges, &nbVertices );
            ...
            /*if ( fac->sculptmap == NULL ) {
                fac->sculptmap = g3dsculptmap_new ( nbVertices );
            } else {
                g3dsculptmap_realloc ( fac->sculptmap, nbVertices );
            }*/

            /*if ( st->ctrl_key ) {
                fac->sculptmap->points[verID].x += (dir->x * 0.0025f);
                fac->sculptmap->points[verID].y += (dir->y * 0.0025f);
                fac->sculptmap->points[verID].z += (dir->z * 0.0025f);
