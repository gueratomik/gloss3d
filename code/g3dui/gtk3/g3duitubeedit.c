...
#define EDITTUBEGENERAL   "Tube"
#define EDITTUBECAPS      "Caps"
...
/******************************************************************************/
static GTK3G3DUITUBEEDIT *gtk3_g3duitubeedit_new ( GTK3G3DUI *gtk3gui ) {
    GTK3G3DUITUBEEDIT *gtk3ted = calloc ( 0x01, sizeof ( GTK3G3DUITUBEEDIT ) );
...
/******************************************************************************/
static void updateGeneralPanel ( GTK3G3DUITUBEEDIT *gtk3ted ) {
    gtk3ted->core.gui->lock = 0x01;

    if ( gtk3ted->core.editedTube ) {
        G3DPRIMITIVE *pri = gtk3ted->core.editedTube;
        TUBEDATASTRUCT *tds = ( TUBEDATASTRUCT * ) pri->data;

        gtk_spin_button_set_value ( gtk3ted->capsEntry     , tds->capx      );
        gtk_spin_button_set_value ( gtk3ted->slicesEntry   , tds->slice     );
        gtk_spin_button_set_value ( gtk3ted->lengthEntry   , tds->length    );
        gtk_spin_button_set_value ( gtk3ted->radiusEntry   , tds->radius    );
        gtk_spin_button_set_value ( gtk3ted->thicknessEntry, tds->thickness );
    }
...
static void lengthCbk ( GtkWidget *widget, gpointer user_data ) {
    GTK3G3DUITUBEEDIT *gtk3ted = ( GTK3G3DUITUBEEDIT * ) user_data;
...
void gtk3_g3duitubeedit_update ( GTK3G3DUITUBEEDIT *gtk3ted ) {
    ...
    if ( gui->sce ) {
        ...
        if ( nbsel ) {
            ...
            if ( g3dobjectlist_checkType ( sce->lsel, G3DTUBETYPE ) ) {
                gtk3ted->core.multi = ( nbsel > 0x01 ) ? 0x01 : 0x00;

                gtk3ted->core.editedTube = ( G3DPRIMITIVE * ) g3dscene_getLastSelected ( sce );
...
/******************************************************************************/
static void createGeneralPanel ( GTK3G3DUITUBEEDIT *gtk3ted,
                                 gint               x,
                                 gint               y,
                                 gint               width,
                                 gint               height ) {
    ...
    gtk3ted->lengthEntry     = ui_createFloatText   ( pan, 
                                                      gtk3ted,
                                                      EDITTUBELENGTH,
                                                      CLASS_MAIN,
                                                      0.0f, FLT_MAX,
    ...
    gtk3ted->radiusEntry     = ui_createFloatText   ( pan, 
                                                      gtk3ted,
                                                      EDITTUBERADIUS,
                                                      CLASS_MAIN,
                                                      0.0f, FLT_MAX,
    ...
    gtk3ted->thicknessEntry  = ui_createFloatText   ( pan, 
                                                      gtk3ted,
                                                      EDITTUBETHICKNESS,
                                                      CLASS_MAIN,
                                                      0.0f, FLT_MAX,
