...
/******************************************************************************/
static void colorchangingcbk ( Widget w, XtPointer client, XtPointer call ) {
    XmColorEditCallbackStruct *ces = ( XmColorEditCallbackStruct * ) call;
    Widget btn = ( Widget ) client;
    ColorEditLabelData *cld;

    XtVaSetValues ( btn, XmNbackground, ces->pixel, NULL );

    XtVaGetValues ( btn, XmNuserData, &cld, NULL );

    if ( cld && cld->func ) {
        cld->func ( ces->pixel, cld->data );
...
static void destroycbk ( Widget w, XtPointer client, XtPointer call ) {
    ...
    /*** Free ColorEditLabelData ***/
    if ( ptr ) free ( ptr );
}

/******************************************************************************/
static void pushcbk ( Widget w, XtPointer client, XtPointer call ) {
    Display *dis = XtDisplay ( w );
    ...
    cld = ( ColorEditLabelData * ) userData;
    ...
    XtAddCallback ( edit, XmNcolorChangingCallback, colorchangingcbk, w );
...
/******************************************************************************/
ColorEditLabelData *coloreditlabeldata_new ( void (*func)( uint32_t, void * ),
                                             void  *data, uint32_t pixel ) {
    int size = sizeof ( ColorEditLabelData );
    ColorEditLabelData *cld = ( ColorEditLabelData * ) calloc ( 0x01, size );

    if ( cld == NULL ) {
        fprintf ( stderr, "coloreditlabeldata_new: mem allocation failed\n" );
...
/******************************************************************************/
void XmColorEditLabelSetColor ( Widget w, uint32_t color ) {
    void *userData;

    XtVaGetValues ( w, XmNuserData, &userData, NULL );

    if ( userData ) {
        ColorEditLabelData *cld = ( ColorEditLabelData * ) userData;
...
/******************************************************************************/
void XmColorEditLabelSetFunc ( Widget w, void (*func)( uint32_t, void * ),
                                         void  *data ) {
    void *userData;
...
/******************************************************************************/
Widget XmCreateColorEditLabel ( Widget parent, char *name, 
                                Dimension x    , Dimension y     ,
                                Dimension width, Dimension height,
                                uint32_t pixel,
                                void (*func)( uint32_t, void * ),
                                void  *data ) {
    ColorEditLabelData *cld = ( func ) ? coloreditlabeldata_new ( func, 
                                                                  data, 
                                                                  pixel ) : NULL;
    ...
    Arg args[] = { { XmNx                 , ( XtArgVal ) x        },
                   ...
                   { XmNforeground        , ( XtArgVal ) pixel    },
                   { XmNuserData          , ( XtArgVal ) cld      },
    ...
    XtAddCallback ( btn, XmNactivateCallback, pushcbk, NULL );
    XtAddCallback ( btn, XmNdestroyCallback, destroycbk, NULL );
